...
public partial class ConsciousBehaviour : BehaviourBase
{
    ...
    public struct MuscleProps
    {
        ...
        /// <summary>
        /// 用于 决定 pinWeight 在肌肉恢复原状的时候应该处于的 范围
        /// 和 动作状态属性对应
        /// </summary>
        /// <returns></returns>
        [Range(0,1)]public float maxPinWeightMlp;
        [Range(0,5)]public float minPinWeightMlp;
    ...
    /// </summary>
    [System.Serializable]
    public class MuscleBasePropsGroup
    {
        public MuscleBasePropsGroup(string name,ActorParts.MuscleGroup mg,MuscleProps mp){

            this.name = name;this.muscleGroup = mg;
            this.props = mp;
        }
        // just for the Editor
        ...
        /// </summary>
        [HideInInspector]
        public ActorParts.MuscleGroup muscleGroup;
        //public ActorParts.MuscleGroup muscleGroup;
	...
	[LargeHeader("StateController Communication")]
    ...
    /// </summary>
    public MuscleBasePropsGroup[] groupOverrides;
    ...
    protected override void OnInitiate()
    {
        ...
        */
        ...
        // 检查 controller 和 puppet 各自的 layer 对于 需要 collider 的 controller 有用
        // foreach (Muscle m in puppetMaster.muscles) {
    ...
    protected override void OnFixedUpdate()
    {
        ...
        if (state != State.Unpinned && !puppetMaster.isKilling)
        {
            ...
            // foreach (MusclePropsGroup MPG in groupOverrides){
            //     foreach(Muscle m in MPG.muscleGroup.muscles){
            //         m.state.pinWeightMlp = Mathf.Clamp(m.state.pinWeightMlp,MPG.props.minPinWeightMlp,MPG.props.maxPinWeightMlp);
            //     }
            // }
        }
    ...
    private void BlendMuscleMapping(int muscleIndex, ref bool to)
    {
        ...
        var props = GetProps(puppetMaster.muscles[muscleIndex]);
    ...
    //         if (groupOverrides[g].muscleTrans.Length > 0)
    //         {
    //             for (int i = 0; i < groupOverrides[g].muscleTrans.Length; i++)
    //             {
    //                 if (i > 0) groupOverrides[g].name += ", ";
    //                 groupOverrides[g].name += groupOverrides[g].muscleTrans[i].ToString();
    //             }
    //         }
    //     }
    // }

    public void InitMuscleGroup(StateController.GroupProp[] groups){
        groupOverrides = new MuscleBasePropsGroup[groups.Length];
        for(int i=0;i<groups.Length;i++){
            groupOverrides[i] = new MuscleBasePropsGroup(groups[i].name,groups[i].MscleGroup,defaults);
            groups[i].muscleBasePropsGroup = groupOverrides[i];
        }
    }
}
